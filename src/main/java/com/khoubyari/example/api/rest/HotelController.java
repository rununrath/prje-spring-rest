...
public class HotelController extends AbstractRestHandler {
    ...
    // add api version, based on sematic versioning 
    @RequestMapping(value="/apiversion",  method = RequestMethod.GET, produces = {"application/json"})
    @ResponseStatus(HttpStatus.OK)
    @ApiOperation(value = "Get api version.", notes = "Return api version.")
    @ResponseBody
    public HashMap<String,String> getVersion(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        result.put("jvmVersion", String.format("%s %s", jvmVendor, jvmVersion));
        
        return result;
    }

    // simulate high resource intensive calculation
    @RequestMapping(value="/intensivecalc",  method = RequestMethod.GET, produces = {"application/json"})
    @ResponseStatus(HttpStatus.OK)
    @ResponseBody
    public HashMap<String,String> intensiveCalc(@RequestParam("loops") String loops) throws Exception {
        long startTime = System.currentTimeMillis();
        long count = Integer.parseInt(loops); //loops by 1E6
        count = count * 1000000;

        double det = 360.0/count;
        for (long i = 0; i < count; i++) {
          Math.tan(Math.toRadians(i * det));
        }
        long endTime = System.currentTimeMillis();
        HashMap<String, String> result = new HashMap<String, String>();
        result.put("count", loops);
